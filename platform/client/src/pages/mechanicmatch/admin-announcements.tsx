...
import type { MechanicmatchAnnouncement } from "@shared/schema";
...
export default function MechanicMatchAdminAnnouncements() {
  const { toast } = useToast();
  const [editingId, setEditingId] = useState<string | null>(null);

  const { data: announcements, isLoading } = useQuery<MechanicmatchAnnouncement[]>({
    queryKey: ["/api/mechanicmatch/admin/announcements"],
  });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: AnnouncementFormValues) => {
      ...
      return apiRequest("POST", "/api/mechanicmatch/admin/announcements", payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/mechanicmatch/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/mechanicmatch/announcements"] });
      form.reset();
      toast({
        title: "Success",
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: AnnouncementFormValues }) => {
      ...
      return apiRequest("PUT", `/api/mechanicmatch/admin/announcements/${id}`, payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/mechanicmatch/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/mechanicmatch/announcements"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return apiRequest("DELETE", `/api/mechanicmatch/admin/announcements/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/mechanicmatch/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/mechanicmatch/announcements"] });
  ...
  const handleEdit = (announcement: MechanicmatchAnnouncement) => {
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Create and manage MechanicMatch announcements
          </p>
        </div>
      </div>
