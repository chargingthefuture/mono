...
import type { LostmailAnnouncement } from "@shared/schema";
...
export default function LostMailAdminAnnouncements() {
  const { toast } = useToast();
  const [editingId, setEditingId] = useState<string | null>(null);

  const { data: announcements, isLoading } = useQuery<LostmailAnnouncement[]>({
    queryKey: ["/api/lostmail/admin/announcements"],
  });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: AnnouncementFormValues) => {
      ...
      return apiRequest("POST", "/api/lostmail/admin/announcements", payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/lostmail/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/lostmail/announcements"] });
      form.reset();
      toast({
        title: "Success",
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: AnnouncementFormValues }) => {
      ...
      return apiRequest("PUT", `/api/lostmail/admin/announcements/${id}`, payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/lostmail/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/lostmail/announcements"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return apiRequest("DELETE", `/api/lostmail/admin/announcements/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/lostmail/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/lostmail/announcements"] });
  ...
  const handleEdit = (announcement: LostmailAnnouncement) => {
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Create and manage LostMail announcements
          </p>
        </div>
      </div>
