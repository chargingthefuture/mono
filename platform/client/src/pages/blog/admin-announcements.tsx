...
import type { BlogAnnouncement } from "@shared/schema";
...
export default function BlogAdminAnnouncements() {
  const { toast } = useToast();
  const [editingId, setEditingId] = useState<string | null>(null);

  const { data: announcements, isLoading } = useQuery<BlogAnnouncement[]>({
    queryKey: ["/api/blog/admin/announcements"],
  });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: AnnouncementFormValues) => {
      ...
      return apiRequest("POST", "/api/blog/admin/announcements", payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/blog/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/blog/announcements"] });
      form.reset();
      toast({
        title: "Success",
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: AnnouncementFormValues }) => {
      ...
      return apiRequest("PUT", `/api/blog/admin/announcements/${id}`, payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/blog/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/blog/announcements"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return apiRequest("DELETE", `/api/blog/admin/announcements/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/blog/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/blog/announcements"] });
  ...
  const handleEdit = (announcement: BlogAnnouncement) => {
  ...
  return (
    <div className="p-6 md:p-8 space-y-8">
      ...
      <div className="flex items-center gap-4">
        <Link href="/apps/blog/admin">
          <Button variant="ghost" size="icon" data-testid="button-back">
            <ArrowLeft className="w-5 h-5" />
          </Button>
        </Link>
        <div>
          <h1 className="text-3xl md:text-4xl font-semibold">Blog Announcements</h1>
          <p className="text-muted-foreground">
            Create and manage announcements for the blog mini-app
          </p>
        </div>
      </div>
