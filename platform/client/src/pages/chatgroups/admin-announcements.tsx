...
export default function ChatGroupsAdminAnnouncements() {
  const { toast } = useToast();
  const [editingId, setEditingId] = useState<string | null>(null);

  const { data: announcements, isLoading } = useQuery<ChatgroupsAnnouncement[]>({
    queryKey: ["/api/chatgroups/admin/announcements"],
  });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: AnnouncementFormValues) => {
      ...
      return apiRequest("POST", "/api/chatgroups/admin/announcements", payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/chatgroups/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/chatgroups/announcements"] });
      form.reset();
      toast({
        title: "Success",
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: AnnouncementFormValues }) => {
      ...
      return apiRequest("PUT", `/api/chatgroups/admin/announcements/${id}`, payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/chatgroups/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/chatgroups/announcements"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return apiRequest("DELETE", `/api/chatgroups/admin/announcements/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/chatgroups/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/chatgroups/announcements"] });
  ...
  const handleEdit = (announcement: ChatgroupsAnnouncement) => {
  ...
  return (
    <div className="p-6 md:p-8 space-y-8">
      <div className="flex items-center gap-4">
        <Link href="/apps/chatgroups/admin">
          <Button variant="ghost" size="icon" data-testid="button-back">
            <ArrowLeft className="w-5 h-5" />
          </Button>
        </Link>
        <div>
          <h1 className="text-3xl md:text-4xl font-semibold">ChatGroups Announcements</h1>
